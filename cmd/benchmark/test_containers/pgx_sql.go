...
func SetupTestDatabasePgx() (testcontainers.Container, *models.PG, error) {
	...
	if _, err = storage.ExecContext(context.Background(), string(text)); err != nil {
		return nil, nil, err
	}

	store := models.NewPG(storage)
